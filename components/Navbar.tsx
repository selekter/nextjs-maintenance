  ...
  return (
    <>
      <button
        ...
        className="fixed left-4 top-4 z-30 grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 shadow-sm md:hidden"
        aria-label="เปิดเมนู"
      >
        <Menu size={21} />
      </button>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-[#102a43] px-4 py-5 text-white shadow-2xl transition-transform duration-300 md:sticky md:top-0 md:h-screen md:translate-x-0 md:shadow-none",
      ...
      >
        ...
        <div className="border-t border-white/10 pt-4">
          {session ? (
            <button
              className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-slate-300 transition hover:bg-rose-500/15 hover:text-rose-200"
            ...
            <Link
              className="block rounded-xl bg-teal-500 px-3 py-3 text-center text-sm font-semibold"
              href="/login"
            >
              เข้าสู่ระบบ
            </Link>
